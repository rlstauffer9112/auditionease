...
export function JudgesEditor({ authFetch, divisionId = null, collapsible, lockedMessage }: Props) {
  ...
  const content = lockedMessage ? (
    <div className="bg-white p-5 sm:p-6 rounded-3xl border border-[#E5E7EB] text-sm text-[#6B7280]">{lockedMessage}</div>
  ) : (
    <div className="bg-white p-5 sm:p-6 rounded-3xl border border-[#E5E7EB] shadow-sm space-y-4">
      ...
      <form onSubmit={handleAdd} className="flex flex-wrap gap-2">
        <input
          ...
          className="flex-1 basis-full sm:basis-0 min-w-0 sm:min-w-[220px] border border-[#E5E7EB] rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#4F46E5] outline-none"
        />
        <button
          type="submit"
          disabled={adding || !email.trim()}
          className="w-full sm:w-auto justify-center flex items-center gap-1.5 bg-[#4F46E5] text-white px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-[#4338CA] disabled:opacity-50"
        >
      ...
      {loading ? (
        ...
        <div className="divide-y divide-[#F3F4F6] border border-[#E5E7EB] rounded-2xl">
          {list.map(j => (
            <div key={j.id} className="flex items-center gap-3 pl-4 pr-2 sm:px-4 py-2.5">
              ...
              <button onClick={() => handleRemove(j)} className="shrink-0 p-2.5 sm:p-1.5 text-[#9CA3AF] hover:text-[#EF4444] hover:bg-[#FEF2F2] rounded-lg" title="Remove">
                <Trash2 size={14} />
              </button>
            </div>
          ))}
